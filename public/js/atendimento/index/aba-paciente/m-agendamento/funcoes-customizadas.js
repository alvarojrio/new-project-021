    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.status_busca == 'sucesso') {
                ...
                // Executa função para buscar dados de agendamentos do paciente
                buscar_agendamentos_paciente(response.cod_pessoa_crypt);
...
function exibir_caixa_paciente() {
    ...
    // Ativa campos de busca do pronto-atendimento
...
function ocultar_caixa_paciente() {
	...
	/*$("#tab_buscar_agendamento").closest('li').addClass('active');

	// Marca conteúdo da aba como ativo com adição de classes
	$("#tab_buscar_agendamento").addClass('active').addClass('in');

	// Marca <li> da aba como inativo com remoção de classes
	$("#aba_novo_agendamento").closest('li').removeClass('active');

	// Marca conteúdo da aba como inativo com remoção de classes
	$("#aba_novo_agendamento").removeClass('active').removeClass('in');*/
    ...
    // Desativa campos de busca do pronto-atendimento
...
function ativar_campos_busca_agenda() {
	...
	if (transferencia_de_consulta == 'ativa') {
	    ...
	    // [Carregamento padrão] Carrega dados em combobox, filtrando por idade e especialidade
	   	carregar_combo_medicos( $('.cxp_idade').html(), $('#cod_especialidade').find(':selected').val(), $('#cod_unidade').find(':selected').val() );
		...
		// [Carregamento padrão] Carrega dados em combobox, filtrando
	    carregar_combo_especialidades( $('.cxp_sexo').data('codigosexo'), $('#cod_unidade').find(':selected').val(), '', $('.cxp_idade').html(), $('.cxp_data_nascimento').html() ); // Recupera o código do sexo através de um data-attribute e coloca na função

	    // [Carregamento padrão] Carrega dados em combobox, filtrando
	   	carregar_combo_medicos( $('.cxp_idade').html(), '', $('#cod_unidade').find(':selected').val() );
...
function desativar_campos_busca_agenda() {
	...
	if (transferencia_de_consulta == 'ativa') {
        ...
        // Oculto div com resultado da busca por agendamentos do paciente
        $('.panel-resultado-busca-agendamentos').hide();
...
/************************************
 #
 # Efetuo requisição para buscar agendamentos de um determinado paciente
 # 
*************************************/ 
function buscar_agendamentos_paciente(cod_pessoa_crypt, origem_cancelamento = false) {
    
    // Exibo div do icone 'processando'
    $('#loading_processando_aba_buscar_agendamento').show();

    // Garanto que a div com resultado da busca por agendamentos do paciente esteja oculta no inicio do processo
    $('.panel-resultado-busca-agendamentos').hide();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.status == 'falha') {          
                ...
                // Exibo div com resultado da busca por agendamentos do paciente
                $('.panel-resultado-busca-agendamentos').show();

                // Caso a função tenha sido chamada pelo procedimento de cancelamento, ou seja, 'origem_cancelamento' é igual a true
                if (origem_cancelamento == true) {
                    ...
                    $.toast({
                        heading: 'Agendamento de consulta cancelado com sucesso!',
                ...
                // Oculto div do icone 'processando'
                $('#loading_processando_aba_buscar_agendamento').hide();
                ...
                // Executa função para desenhar tabela de dados e ativar DataTable nela
                desenhar_tabela_agendamentos_paciente(dados_retornados);
...
/************************************
 #
 # Efetuo o desenho da tabela onde serão listados os agendamentos do paciente, ativa a DataTable e preenche com dados
 # 
*************************************/ 
function desenhar_tabela_agendamentos_paciente(dados_recebidos) {
    ...
    // Instancio DataTable
    tabela_buscar_agendamento = $('#tabela_buscar_agendamento').DataTable({
        ...
        columns: [
            ...
            { "data": "periodo", "name": "periodo", "width": "19%", "searchable": true, "sortable": true },
            { "data": "tipo_consulta", "name": "tipo_consulta", "width": "19%", "searchable": true, "sortable": true },       
            { "data": "status_consulta", "name": "status_consulta", "width": "19%", "searchable": true, "sortable": true },
            { "data": "btn_visualizar", "name": "btn_visualizar", "width": "3%", "searchable": false, "sortable": false },
