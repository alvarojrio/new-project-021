...
$(document).ready(function() {
    ...
    *************************************/ 
    $(document).on('change', '#cod_unidade', function() { 
        ...
        if (transferencia_de_consulta == 'ativa') {
            ...
            // Carrega dados em combobox, filtrando por sexo e unidade
            carregar_combo_especialidades( $('.cxp_sexo').data('codigosexo'), $(this).find(':selected').val(), '', $('.cxp_idade').html(), $('.cxp_data_nascimento').html() ); // parametros: sexo, unidade 
    ...
    *************************************/
    $(document).on('click', '#tab_buscar_agendamento', function(e) {
        ...
        // Checa se existe algum paciente selecionado através do código da pessoa
        if (cod_pessoa_crypt != '') {

            // Executa função para buscar dados de agendamentos do paciente
            buscar_agendamentos_paciente(cod_pessoa_crypt);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status == 'validacao') {
                    ...
                    // Executa função para buscar dados de agendamentos do paciente. Colocando true no segundo parametro indico que a origem foi o modal de cancelamento
                    buscar_agendamentos_paciente(cod_pessoa_crypt, true);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status_requisicao == 'erro') {
                    ...
                    $('#modal-resumo-consulta-agendada').modal({keyboard: true});
